...
const StoryEdit = (props) => {
  const [who, setWho] = useState(props.whoEdit);
  const [image, setImage] = useState('');
  const [description, setDescription] = useState(props.descriptionEdit);
  const [sexDate, setSexDate] = useState(props.sexDateTotalEdit);
  const [rating, setRating] = useState(props.ratingEdit);
  const [phoneNumber, setPhoneNumber] = useState(props.phoneNumberEdit);
  function validateForm() {
    return (
      who.length > 0 && description.length > 0 && sexDate !== '' && rating > 0
    );
  }
